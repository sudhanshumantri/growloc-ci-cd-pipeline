...
export default function zoneReducer (state = INITIAL_STATE, action = {}) {
  const zoneDashboardZoneTaskList = state.toJS().zoneDashboardZoneTaskList
  const zoneTaskList = state.toJS().zoneTaskList
  ...
  switch (action.type) {
    ...
    case ADD_ZONE_TASK_COMMENT_SUCCESS: {
      ...
      zoneTaskList.tasks[ZoneTaskRowIndex].TasksHistory.push({
        ...action.data,
        user
      })
      return state
