...
import {
  FETCH_SENSOR_DATA_PUSHER_REQUEST,
  FETCH_SENSOR_DATA_PUSHER_FAILURE,
  FETCH_SENSOR_DATA_PUSHER_SUCCESS
...
export default function pusherReducer (state = INITIAL_STATE, action = {}) {
  switch (action.type) {
    ...
    case FETCH_SENSOR_DATA_PUSHER_SUCCESS: {
      const sensorData = state.toJS().pusherData
      const idx = sensorData.findIndex(
        (obj) => obj.device_id === action.data.device_id
      )
      if (idx > -1) {
