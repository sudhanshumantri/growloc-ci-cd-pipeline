...
export default function ZoneDashboard ({
  ...
  const headerDropwdown = true

  const handleChangeCropSchedulePagination = (queryParams) => {
    fetchZoneDashboardZoneCropSchdule({ zoneId, queryParams })
  }
  ...
  const handleChange = (event) => {
    ...
    setMonth(value)
    fetchFarmDashboardHarvest({ zoneId, month: value })
  }
  ...
  const handlePlatformChange = (event, newPlatform) => {
    ...
    switch (newPlatform) {
      ...
      case 'taskTat':
        ...
        break
      case 'CapacityEfficiency':
        fetchFarmDashboardHarvest({ zoneId, month })
  ...
  const rendeLattestSensorDataByID = () => {
    ...
    if (!data || !data[0] || !data[0].payload) {
      return <span>No sensor data available</span>
    }
    const rows = [
      {
        name: 'Temperature',
  ...
  return (
    <div>
      ...
      {isFarmZoneSensorDataLoading && (
        <Loader title='Fetching Zone sensors Data' />
      )}
      <div className='page-container'>
        <Grid container spacing={2}>
          <ZoneHeaderInformation
            zoneDashboardZoneInfoList={zoneDashboardZoneInfoList}
          />
          <TabContext value={value}>
            <Grid item xs={12} sm={12} md={12}>
              ...
              <TabPanel value='6'>
                <ZoneInformation farmZoneList={farmZoneList} />
              </TabPanel>
            </Grid>
          </TabContext>
        </Grid>
