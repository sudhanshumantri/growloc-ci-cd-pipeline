...
import {
  callAddCropToLifecycle,
  callCropLifecycleTransition,
  callfetchAllCropsLifecycle,
  callfetchCropsLifecycleDetails,
  callUpdateCropCycleParameters,
  callUpdateCropCycleDetails,
  callUpdateCropToLifecycleSchedule
} from '../utils/api'
...
export function * fetchAllCropsLifecycle ({ data }) {
  const responseData = yield call(callfetchAllCropsLifecycle, data)
...
export function * addCropToLifecycle ({ data }) {
  const responseData = yield call(callAddCropToLifecycle, data)
  if (responseData?.status === 200 && responseData.data.status) {
    yield put(addCropToLifecycleSuccess(responseData.data.data))
    addNotification(
      'Add Crop Lifecyle Added Successfully',
      5000,
      true,
      'success'
    )
...
export function * cropLifecycleTransition ({ data }) {
  const responseData = yield call(callCropLifecycleTransition, data)
  if (responseData?.status === 200 && responseData.data.status) {
...
export function * fetchCropsLifecycleDetails ({ data }) {
  const responseData = yield call(callfetchCropsLifecycleDetails, data)
  if (responseData?.status === 200 && responseData.data.status) {
...
export function * updateCropToLifecycleParameters ({ data }) {
  const responseData = yield call(callUpdateCropCycleParameters, data)
  if (responseData?.status === 200 && responseData.data.status) {
...
export function * updateCropToLifecycleSchedule ({ data }) {
  const responseData = yield call(callUpdateCropToLifecycleSchedule, data)
  if (responseData?.status === 200 && responseData.data.status) {
...
export function * updateCropToLifecycleDetails ({ data }) {
  const responseData = yield call(callUpdateCropCycleDetails, data)
  if (responseData?.status === 200 && responseData.data.status) {
...
export function * cropLifeCycleSagas () {
  yield all([
    ...
    takeLatest(
      ...
      updateCropToLifecycleParameters
    ),
    takeLatest(
