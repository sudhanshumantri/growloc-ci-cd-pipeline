...
export function * updateUserProfile ({ data }) {
  ...
  const responseData = yield call(callupdateUserProfile, remaingData)
...
export function * updatePhoneNumber ({ data }) {
  const responseData = yield call(callupdateChangePhone, data)
  if (responseData?.status === 200 && responseData.data.status) {
    yield put(updateUserPhoneOrPasswordSuccess(responseData.data.data))
    addNotification(
      ' Phone Number Updated. You would be logged out and use the new Phone Number to login again.',
      5000,
      true,
      'success'
    )
...
export function * updatePassword ({ data }) {
  const responseData = yield call(callChangePasswordHandler, data)
  if (responseData?.status === 200 && responseData.data.status) {
    yield put(updateUserNewPasswordSuccess(responseData.data.data))
    addNotification(
      ' Password Updated. You would be logged out and use the new password to login again.',
      5000,
      true,
      'success'
    )
